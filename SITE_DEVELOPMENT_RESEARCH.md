...
> **Статус:** В работе. Стабилизация завершена, Astro Prefetch и Partytown внедрены как первые safe improvements.
...
- **Фактический статус на 2026-06-08:** внедрено через `@astrojs/partytown@2.1.7` + `forward: ['ym']`. Яндекс.Метрика по-прежнему грузится только после cookie accept: `CookieBanner` динамически добавляет `type="text/partytown"` script и отправляет `ptupdate`. Дополнительно пришлось ужесточить consent-gate, потому что после forward `window.ym` существует уже до загрузки реальной Метрики.
...
- **Фактический статус на 2026-06-08:** протестировано и откатано. В проекте `astro-min` вызывал hydration errors у Astro islands (`RequestModal`, `CookieBanner`) даже после смягчения опций и успешного `build`/`geo-check`. Для текущего сайта пакет помечен как `не подходит` до выхода совместимого фикса или отдельного изолированного workaround.
...
- **Фактический статус на 2026-06-08:** внедрено частично и безопасно. Создан локальный набор `src/icons/*`, а на `astro-icon` переведены повторяющиеся системные иконки в `Header`, `Footer`, `RouteMapOverlay`, `FinalCTA`. Уникальные декоративные SVG и брендовые mark-иконки пока оставлены inline как отдельный возможный этап.
...
- **Фактический статус на 2026-06-08:** внедрено в `astro.config.mjs` через `prefetchAll: true` и `defaultStrategy: 'hover'`; выбрана осторожная стратегия без новых зависимостей и без вмешательства в cookie/analytics слой.
...
- **Фактический статус на 2026-06-08:** server audit выполнен. На AMS Server HTML уже сжимается обычным `gzip on`, но JS/CSS не сжимаются, потому что `gzip_types` для статики закомментирован. Nginx собран с `gzip_static_module`, однако `gzip_static` не включён, Brotli отсутствует. Пока нет возможности безопасно править server config из текущего deploy-доступа, поэтому внедрение `PlayForm/Compress` отложено как `требует infra-шага вне репозитория`.
...
1. **Partytown + Яндекс.Метрика** — следующий большой performance-этап после prefetch.
   - ✅ Совместимость с текущим `analytics.ts` проверена.
   - ✅ `forward: ['ym']` настроен.
   - ⏳ Осталось повторить network-проверку в окружении, где задан `PUBLIC_YM_COUNTER_ID`.

2. **Astro Prefetch** — бесплатное улучшение UX.
   - ✅ Добавлен в `astro.config.mjs`.
   - Проверить desktop/mobile навигацию и отсутствие лишних регрессий.

### Среднесрочные (на следующей неделе)
3. **astro-min** — протестирован, но пока не подходит.
4. **astro-icon** — частично внедрён.
