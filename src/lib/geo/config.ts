...
export const GEO_FAQ = [
  {
    ...
    a: 'Военный навигатор — это сервис, который помогает разобраться в задаче покупки по военной ипотеке, определить ограничения и только потом перейти к выбору квартиры или новостройки.',
  },
  {
    q: 'Что происходит на первом разборе?',
    a: 'На первом разборе уточняются цель покупки, бюджет, ограничения программы, банк, сроки и формат сделки. После этого становится понятнее, какие варианты и какой следующий шаг реально подходят.',
  },
  {
  ...
  {
    q: 'Можно ли пройти разбор дистанционно?',
    a: 'Да, первый разбор можно провести онлайн. Дальше отдельно определяется, какие этапы сделки проходят удалённо, а где понадобится личное участие или доверенность.',
  },
  {
  ...
  },
  {
    ...
    a: 'Первичный разбор нужен, чтобы сначала понять задачу и ограничения, а не идти в подбор вслепую. Точные условия лучше уточнять на странице сервиса или при обращении, потому что формат следующего шага зависит от вашей ситуации.',
  },
] as const;
